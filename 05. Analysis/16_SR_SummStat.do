...
lab var lfp_temp_7d "Temp Labor"
...
lab var formal_7d "Formal"


eststo sumstat: 	 qui estpost su 	$SR_outcome 
...
eststo year2010: 	estpost su 		$SR_outcome   if year == 2010
eststo year2016: 	estpost su 		$SR_outcome   if year == 2016
*eststo diff_year:  estpost ttest 	$SR_outcome  , by(year) unequal

*diff_year
esttab 	year2010 year2016  ///
		using "$out_analysis/SR_REF_SummStat_Year.tex", ///
		cells("mean(pattern(1 1) fmt(2) label(Mean)) sd(pattern(1 1) label(SD)) count(pattern(1 1) fmt(0))") ///
		label nonumber booktabs replace gaps star(* 0.10 ** 0.05 *** 0.01) ///
		mtitles("\textbf{Year 2010}" "\textbf{Year 2016}") ///
...
esttab 	year2010 year2016 diff_year ///
		using "$out_analysis/SR_REF_SummStat_Year_diffmean.tex", ///
...
lab var employed_olf_3m "Employed"
lab var unemployed_3m "Unemployed"
lab var lfp_empl_3m "Wage Employee"
lab var lfp_temp_3m "Temp Labor"
lab var lfp_employer_3m "Employer"
lab var lfp_se_3m "Self-Employed"
lab var act_ag_3m "Agriculture"
lab var act_manuf_3m "Manufacturing"
lab var act_com_3m "Commerce"
lab var act_serv_3m "Services"
lab var ln_trwage_3m "Monthly Wage (ln)"
lab var ln_hrwage_main "Hourly Wage (ln)"
lab var ln_whpw_w_3m "Work Hours p.w. (ln)"
lab var formal_3m "Formal"


eststo sumstat_3m: 	 qui estpost su 	$SR_outcome_3m 

esttab 	sumstat_3m ///
		using "$out_analysis/SR_REF_SummStat_3m.tex", ///
		cells(" count(fmt(0))  mean(fmt(2) label(Mean)) sd(fmt(2) label(SD)) min(fmt(2)) max(fmt(2))") ///
		label  booktabs replace gaps star(* 0.10 ** 0.05 *** 0.01) ///
		collabels(\multicolumn{1}{l}{{Obs}} \multicolumn{1}{c}{{Mean}} \multicolumn{1}{c}{{Std.Dev.}} \multicolumn{1}{l}{{Min}} \multicolumn{1}{l}{{Max}}) 
...
eststo year2010_3m: 	estpost su 		$SR_outcome_3m   if year == 2010
eststo year2016_3m: 	estpost su 		$SR_outcome_3m   if year == 2016
*eststo diff_year:  estpost ttest 	$SR_outcome  , by(year) unequal

*diff_year
esttab 	year2010_3m year2016_3m  ///
		using "$out_analysis/SR_REF_SummStat_Year_3m.tex", ///
		cells("mean(pattern(1 1) fmt(2) label(Mean)) sd(pattern(1 1) label(SD)) count(pattern(1 1) fmt(0))") ///
		label nonumber booktabs replace gaps star(* 0.10 ** 0.05 *** 0.01) ///
		mtitles("\textbf{Year 2010}" "\textbf{Year 2016}") ///
		collabels(\multicolumn{1}{c}{{Mean}} \multicolumn{1}{c}{{Std.Dev.}} \multicolumn{1}{l}{{Obs}}) 
