...
**** OLS *****
  foreach outcome of global SR_outcome {
    qui xi: reg `outcome' $SR_treat_var_ref inter_gender gender ///
                age age2 ///
                i.district_iid i.year  ///
            [pw = $SR_weight],  ///
            cluster(district_iid) robust 
    codebook `outcome', c
    estimates table, k($SR_treat_var_ref inter_gender gender) star(.1 .05 .01) b(%7.4f) 
    estimates table, b(%7.4f) se(%7.4f) stats(N r2_a) k($SR_treat_var_ref inter_gender gender) 
...
preserve
  foreach outcome of global SR_outcome {
       qui reghdfe `outcome' $SR_treat_var_ref inter_gender gender ///
                age age2 ///  
                [pw = $SR_weight], ///
      ...
      foreach y in `outcome' $SR_treat_var_ref  inter_gender gender ///
                age age2  {
      ...
      qui reg `outcome' $SR_treat_var_ref inter_gender gender  age age2 /// 
       [pw = $SR_weight], cluster(district_iid) robust
      codebook `outcome', c
      estimates table,  k($SR_treat_var_ref  inter_gender gender) star(.1 .05 .01) 
      estimates table, b(%7.4f) se(%7.4f) stats(N r2_a) k($SR_treat_var_ref  inter_gender gender) 
      estimates store REFGEN_YI_`outcome', title(Model `outcome')

      drop `outcome' inter_gender gender age age2   $SR_treat_var_ref smpl
      
      foreach y in `outcome' inter_gender gender age age2  $SR_treat_var_ref  {
        rename o_`y' `y' 
      } 
    }
restore
...
****************************************************
     ...
     coefplot (REFGEN_YD_employed_olf_$rp), bylabel(Employed)  ///
            ...
            keep($SR_treat_var_ref inter_gender)
...
**** OLS *****
  foreach outcome of global SR_outcome {
    qui xi: reg `outcome' $SR_treat_var_ref inter_urban urban ///
                $SR_controls  ///
                i.district_iid i.year  ///
            [pw = $SR_weight],  ///
            cluster(district_iid) robust 
    codebook `outcome', c
    estimates table, k($SR_treat_var_ref inter_urban urban) star(.1 .05 .01) b(%7.4f) 
    estimates table, b(%7.4f) se(%7.4f) stats(N r2_a) k($SR_treat_var_ref inter_urban urban) 
...
preserve
  foreach outcome of global SR_outcome {
       qui reghdfe `outcome' $SR_treat_var_ref inter_urban urban ///
                $SR_controls ///  
                [pw = $SR_weight], ///
                absorb(year indid_2010) ///
                cluster(district_iid) 
        ...
        qui replace smpl=1 if e(sample)==1
        * Then I partial out all variables
      foreach y in `outcome' $SR_treat_var_ref  inter_urban urban ///
                $SR_controls {
        qui reghdfe `y' [pw = $SR_weight] if smpl==1, absorb(year indid_2010) residuals(`y'_c2wr)
        rename `y' o_`y'
        rename `y'_c2wr `y'
      ...
      qui reg `outcome' $SR_treat_var_ref inter_urban urban $SR_controls /// 
       [pw = $SR_weight], cluster(district_iid) robust
      codebook `outcome', c
      estimates table,  k($SR_treat_var_ref  inter_urban urban) star(.1 .05 .01) 
      estimates table, b(%7.4f) se(%7.4f) stats(N r2_a) k($SR_treat_var_ref  inter_urban urban) 
      estimates store REFURB_YI_`outcome', title(Model `outcome')

      drop `outcome' inter_urban urban $SR_controls  $SR_treat_var_ref smpl
      
      foreach y in `outcome' inter_urban urban $SR_controls $SR_treat_var_ref  {
        rename o_`y' `y' 
      } 
    }
restore
...
coefplot (REFURB_YD_employed_olf_$rp), bylabel(Employed)  ///
             ...
             byopts(graphregion(color(white)) bgcolor(white) ///
              title("Effect Sizes: Urban", size(small))) ///
                ...
                region(lstyle(none)) size(vsmall) cols(4) ring(0) )  ///
            levels(99.9 99 95) ciopts(lwidth(*1) lcolor(*.6)) xla(none) xtitle("") xsc(noline) ///
            keep($SR_treat_var_ref inter_urban)
...
**** OLS *****
  foreach outcome of global SR_outcome {
    qui xi: reg `outcome' $SR_treat_var_ref inter_educ bi_education ///
                $SR_controls  ///
                i.district_iid i.year  ///
            [pw = $SR_weight],  ///
            cluster(district_iid) robust 
    codebook `outcome', c
    estimates table, k($SR_treat_var_ref inter_educ bi_education) star(.1 .05 .01) b(%7.4f) 
    estimates table, b(%7.4f) se(%7.4f) stats(N r2_a) k($SR_treat_var_ref inter_educ bi_education) 
...
preserve
  foreach outcome of global SR_outcome {
       qui reghdfe `outcome' $SR_treat_var_ref inter_educ bi_education ///
                $SR_controls ///  
                [pw = $SR_weight], ///
                absorb(year indid_2010) ///
                cluster(district_iid) 
        ...
        qui replace smpl=1 if e(sample)==1
        * Then I partial out all variables
      foreach y in `outcome' $SR_treat_var_ref  inter_educ bi_education ///
                $SR_controls {
        qui reghdfe `y' [pw = $SR_weight] if smpl==1, absorb(year indid_2010) residuals(`y'_c2wr)
        rename `y' o_`y'
        rename `y'_c2wr `y'
      ...
      qui reg `outcome' $SR_treat_var_ref inter_educ bi_education $SR_controls /// 
       [pw = $SR_weight], cluster(district_iid) robust
      codebook `outcome', c
      estimates table,  k($SR_treat_var_ref  inter_educ bi_education) star(.1 .05 .01) 
      estimates table, b(%7.4f) se(%7.4f) stats(N r2_a) k($SR_treat_var_ref  inter_educ bi_education) 
      estimates store REFEDU_YI_`outcome', title(Model `outcome')

      drop `outcome' inter_educ bi_education $SR_controls  $SR_treat_var_ref smpl
      
      foreach y in `outcome' inter_educ bi_education $SR_controls $SR_treat_var_ref  {
        rename o_`y' `y' 
      } 
    }
restore
    ...
    coefplot (REFEDU_YD_employed_olf_$rp), bylabel(Employed)  ///
             ...
             byopts(graphregion(color(white)) bgcolor(white) ///
              title("Effect Sizes: Education", size(small))) ///
                ...
                region(lstyle(none)) size(vsmall) cols(4) ring(0) )  ///
            levels(99.9 99 95) ciopts(lwidth(*1) lcolor(*.6)) xla(none) xtitle("") xsc(noline) ///
            keep($SR_treat_var_ref inter_educ)
